...
void controlTick() {
      ...
      switch (IRdata) {
        ...
        case BUTT_RIGHT:
          ...
          switch (navPos) {
            ...
            case 1:   // выбор режима
              if (IRdata == BUTT_RIGHT) {
                incr(&modeNum, 1, MODE_AMOUNT);    // modeNum +1 огранич MODE_AMOUNT
              } else if (IRdata == BUTT_LEFT) {
                incr(&modeNum, -1, MODE_AMOUNT);    // modeNum -1 огранич 0
              }
              changeMode();
              break;
            case 2:   // настройки
  ...
  if (eeprFlag && millis() - eeprTimer > EEPR_TIME) {
    ...
    if (btnControl) {
      ...
      EEPROM_UPD_BYTE(201, btnColor);
      EEPROM_UPD_WORD(202, thisBright);
    } else {
      EEPROM_UPD_BYTE(PRESET_ADDR, presetNum);
      writeSettings();
    }
  }
}
